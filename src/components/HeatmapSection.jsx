...
export default function HeatmapSection({ riskPoints, riskColor }) {
  return (
    <section
      id="heatmap"
      className="bg-slate-900/50 rounded-xl sm:rounded-2xl border border-slate-800 p-4 sm:p-6 lg:p-8 shadow-xl relative"
    >
      ...
      <div className="grid grid-cols-1 xl:grid-cols-4 gap-6 sm:gap-8">
        <div className="xl:col-span-3 bg-slate-800/50 rounded-lg sm:rounded-xl border border-slate-700 p-4 sm:p-6 relative">
          <div className="bg-slate-900 rounded-lg border border-slate-700 p-4 sm:p-6">
            <div className="relative w-full h-64 sm:h-80 lg:h-[500px] rounded-lg overflow-hidden mb-4 sm:mb-6">
                ...
                >
                  ...
                  {riskPoints.map((point, idx) => (
                    ...
                    >
                      <Tooltip>
                        {point.label} (
                        {point.risk.charAt(0).toUpperCase() + point.risk.slice(1)}{" "}
                        Risk)
                      </Tooltip>
                    </CircleMarker>
                  ))}
